...
class Themify_Builder_Component_Row{
    ...
    public static function clickable_component(array &$attr, array $settings) {
        if (!empty($settings['_link'])) {
            ...
            if ( isset( $settings['_link_n'] ) ) {
                if ( $settings['_link_n'] === 'yes' ) {
                    $attr['data-tb_link_new'] = '1';
                } else if ( $settings['_link_n'] === 'lb' ) {
                    $attr['data-tb_link'] = add_query_arg( 'post_in_lightbox', 1, $attr['data-tb_link'] );
                    $attr['data-tb_lb'] = 1;
                    $settings = $settings + [
                        '_link_lw' => '',
                        '_link_lh' => '',
